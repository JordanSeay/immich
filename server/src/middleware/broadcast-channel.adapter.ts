...
class BroadcastChannelAdapter extends ClusterAdapterWithHeartbeat {
  ...
  private readonly responseChannel: BroadcastChannel;
  ...
  constructor(nsp: any, opts?: Partial<ClusterAdapterOptions>) {
    ...
    this.responseChannel = new BroadcastChannel(`${BC_CHANNEL_NAME}#${this.uid}`);
    this.responseChannel.addEventListener('message', (event: MessageEvent<BroadcastChannelPayload>) => {
      const msg = event.data;
      if (msg.sourceUid === this.uid) {
        return;
      }
  ...
  override doPublishResponse(requesterUid: ServerId, response: ClusterResponse): Promise<void> {
    const target = new BroadcastChannel(`${BC_CHANNEL_NAME}#${requesterUid}`);
    try {
      target.postMessage({
        type: 'response',
        sourceUid: this.uid,
